...
_CONFIG_SCHEMA = [
    ...
    ['Terminal', [
        ...
        ['terminal_reset_minutes',              'int',    'Idle Reset (min)',       'Restart shell after this many minutes idle (0 = off); skips busy tabs'],
        ['terminal_log_enabled',                'bool',   'Session Logging',       'Append ANSI-stripped output to a rotating log file per tab'],
        ['terminal_log_dir',                    'str',    'Log Directory',         'Where session log files are written'],
        ['terminal_notes_file',                 'str',    'Notes File',            'Plain text file opened by Notes mode'],
    ]],
    ['Alerts', [
        ['terminal_alert_health_interval',      'int',    'Health Check Interval (s)', 'How often to check throttle, failed units, storage, and network'],
        ['terminal_alert_throttle',             'bool',   'Throttle & Voltage Alerts', 'Alert on Pi under-voltage or thermal throttle (vcgencmd)'],
        ['terminal_alert_failed_units',         'bool',   'Failed Unit Alerts',    'Alert when systemd units enter a failed state'],
        ['terminal_alert_storage_health',       'bool',   'Storage Health Alerts', 'Alert if the root filesystem is remounted read-only'],
        ['terminal_alert_network',              'bool',   'Network Alerts',        'Alert when the network is unreachable (ping check)'],
        ['terminal_alert_network_host',         'str',    'Network Ping Target',   'Host to ping (blank = auto-detect default gateway)'],
        ['terminal_alert_network_fails',        'int',    'Network Fail Streak',   'Consecutive missed pings before alerting'],
        ['terminal_alert_ssh_logins',           'bool',   'SSH Login Alerts',      'Alert on new SSH sessions (startup baseline is captured automatically)'],
    ...
    ['Screensaver', [
        ...
        ['screensaver_mode',            'select', 'Mode',                 ['static', 'cycle', 'tiles']],
        ...
        ['screensaver_tiles_weather_location', 'str',  'Weather Location',   'Tiles mode: city or zip code (uses wttr.in)'],
    ...
    ['Startup & Web', [
        ...
        ['preview_server_pin',      'str',    'Web PIN',               'PIN-gates mutating endpoints (settings, input, clipboard). Empty = open'],
...
<header><a class="back" href="/">&#8592; Back</a><h1>Logs</h1><span></span></header>
...
function load(){
  ...
  fetch("/logs.json?lines="+n).then(function(r){return r.json();})
   .then(function(d){
...
<header><a class="back" href="/">&#8592; Back</a><h1>Controls</h1><span></span></header>
...
<header><a class="back" href="/">&#8592; Back</a><h1>Push a card</h1><span></span></header>
<div class="card">
...
function push(){
  ...
  fetch("/card",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
   .then(function(r){return r.json();})
...
def _make_handler(bmp_path: str, input_queue: queue.Queue,
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _handle_action(self):
            """POST /action — a quick action. PIN-gated (see do_POST).
        ...
        def _serve_logs(self):
            """GET /logs.json?lines=N — recent journald lines for this service. PIN-gated (see do_GET)."""
