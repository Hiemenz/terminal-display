...
class ShellMixin:
    ...
    @staticmethod
    def _write_view_image_script(bindir: str, port: int) -> None:
        """Write the `view-image <path>` command that pushes an image to the panel."""
        script = (
            '#!/bin/sh\n'
            '# Push an image file to the e-ink panel in 4-gray mode.\n'
            '# Auto-generated by eink_terminal_app.py — edits will be overwritten on launch.\n'
            'if [ -z "$1" ]; then\n'
            '  echo "usage: view-image <file>" >&2; exit 1\n'
            'fi\n'
            f'curl -sf -X POST http://localhost:{port}/image \\\n'
            '  -H "Content-Type: application/json" \\\n'
            '  -d "{\\"path\\":\\"$(realpath \\"$1\\")\\"}" > /dev/null\n'
            'if [ $? -ne 0 ]; then\n'
            '  echo "view-image: server not reachable (is eink-display running?)" >&2; exit 1\n'
            'fi\n'
        )
        path = os.path.join(bindir, 'view-image')
        with open(path, 'w') as f:
            f.write(script)
        os.chmod(path, 0o755)

    @staticmethod
    def _write_restart_script(bindir: str) -> None:
        """Write the `restart-display` command that restarts the systemd service."""
        script = (
            '#!/bin/sh\n'
            '# Restart the eink-display systemd service.\n'
            '# Auto-generated by eink_terminal_app.py — edits will be overwritten on launch.\n'
            'sudo /usr/bin/systemctl restart eink-display\n'
        )
        path = os.path.join(bindir, 'restart-display')
        with open(path, 'w') as f:
            f.write(script)
        os.chmod(path, 0o755)
    ...
    def _install_command_scripts(self):
        ...
        try:
            ...
            self._write_view_image_script(bindir, port)
            self._write_restart_script(bindir)
            names = ('settings', 'eink', 'clear-eink', 'notes', 'llmchat',
                     'terminal', 'commands', 'view-image', 'restart-display')
        except OSError as e:
